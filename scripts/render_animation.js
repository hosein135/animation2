...
function drawableElements(drawables, cam, lights) {
  ...
  for (const d of drawables) {
    if (d.kind === "ellipse") {
      ...
      const rx = projectRadius(cam, d.radii[0], d.center);
      const ry = projectRadius(cam, Math.max(d.radii[1], d.radii[2]) * 0.85, d.center);
      ...
      // Back-face cull soft: still draw distant faces faintly
      ...
      elems.push({
        ...
        cy: c.y,
        rx: Math.max(2, R),
        ry: Math.max(2, R * 0.55),
        fill: "none",
        stroke: shade(d.material, [0.1, -0.9, 0.2], lights),
        strokeWidth: Math.max(1.5, r * 2),
        name: d.name,
      });
    }
  }
  ...
  for (const e of elems) {
    if (e.type === "ellipse") {
      if (e.fill === "none") {
        ...
        parts.push(
          `<ellipse cx="${e.cx.toFixed(2)}" cy="${e.cy.toFixed(2)}" rx="${e.rx.toFixed(2)}" ry="${e.ry.toFixed(2)}" fill="${e.fill}" opacity="0.95"/>`,
        );
      }
    } else if (e.type === "polygon") {
      parts.push(`<polygon points="${e.points}" fill="${e.fill}" stroke="${e.fill}" stroke-width="0.4" opacity="0.96"/>`);
    } else if (e.type === "line") {
      parts.push(
